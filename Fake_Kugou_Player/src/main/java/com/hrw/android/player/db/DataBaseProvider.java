...
/**
 * 定义内容提供者是为了程序间共享数据，在这里是没必要的，直接操作数据库即可。
 */
...
public class DataBaseProvider extends ContentProvider
{
	...
	private static final int PLAYLIST = 1;//处理播放列表

	private static final int AUDIO_LIST = 2;//处理歌曲

	// 添加2个Uri
	static
	{
		//对应数据库的 播放列表的表格
		URI_MATCHER.addURI(UriConstant.AUTHORITY, UriConstant.PLAYLIST_PATH, PLAYLIST);
		//对应数据库的 歌曲的表格
		URI_MATCHER.addURI(UriConstant.AUTHORITY, UriConstant.AUDIO_LIST_PATH, AUDIO_LIST);
	}
